...
export default function SignUpClient() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (signInRes?.error) setSuccess(true);
        ...
        <div className="backdrop-blur-xl bg-white/[0.06] border border-white/[0.12] rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="p-8">
            <div className="flex flex-col items-center mb-7">
              ...
              <h1 className="text-xl font-bold text-white tracking-tight mb-0.5">Create account</h1>
              <p className="text-sm text-white/40">Join Qii Services today</p>
            </div>
            ...
            {success ? (
              <div className="text-center space-y-4 py-4">
                ...
                <Link href="/auth/login" className="inline-block text-cyan-400 hover:text-cyan-300 text-sm font-medium transition-colors">Go to Login →</Link>
              </div>
            ) : (
              <>
                <button id="google-signup-btn" onClick={handleGoogle} disabled={isGoogleLoading}
                  className="w-full flex items-center justify-center gap-3 px-5 py-3 rounded-xl font-semibold text-sm transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 mb-4"
                  ...
                  <span className="text-slate-800">{isGoogleLoading ? "Redirecting…" : "Sign up with Google"}</span>
                </button>
                ...
                <form onSubmit={handleSignup} className="space-y-3">
                  <div>
                    <label className="block text-xs font-medium text-white/50 mb-1.5 ml-0.5">Full Name</label>
                    <input id="signup-name" type="text" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="John Doe"
                  ...
                  <div>
                    ...
                    <input id="signup-email" type="email" required value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="you@example.com"
                      className="w-full px-4 py-2.5 rounded-xl bg-white/[0.07] border border-white/[0.12] text-white placeholder-white/20 text-sm outline-none focus:border-cyan-400/60 focus:bg-white/[0.10] transition-all" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-white/50 mb-1.5 ml-0.5">Password</label>
                    <div className="relative">
                      <input id="signup-password" type={showPassword ? "text" : "password"} required value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} placeholder="Min. 8 characters"
                      ...
                      <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-white/30 hover:text-white/60 transition-colors" tabIndex={-1}><EyeIcon open={showPassword} /></button>
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-white/50 mb-1.5 ml-0.5">Confirm Password</label>
                    <input id="signup-confirm" type={showPassword ? "text" : "password"} required value={form.confirm} onChange={(e) => setForm({ ...form, confirm: e.target.value })} placeholder="Re-enter password"
                      className="w-full px-4 py-2.5 rounded-xl bg-white/[0.07] border border-white/[0.12] text-white placeholder-white/20 text-sm outline-none focus:border-cyan-400/60 focus:bg-white/[0.10] transition-all" />
                  </div>
                  ...
                  <button id="signup-submit-btn" type="submit" disabled={isLoading}
                    ...
                    {isLoading ? <span className="flex items-center justify-center gap-2"><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Creating account…</span> : "Create Account"}
                  </button>
                </form>
